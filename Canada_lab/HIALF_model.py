...
class HIALF:
		...
		for i in range(2,Max_len+1):
			...
			for item in len_of_every_item.keys():
				if(i<=len_of_every_item[item]):
					...
					previous_e = (float)(sum(get_previous_i['Rating'].loc[get_previous_i['MovieID']==item])-which_rate)/float(i-1)
	...
	def train_model(self,sequence_info,isTrain):
		...
		R_predict_second_part = tf.matmul(tf.matmul(tf.transpose(bu_OneHot), tf.matmul(Xu, tf.transpose(Yp))), bp_OneHot) # X*Y.T
		...
		Vl = tf.Variable(tf.random_normal([len(e_l)],mean=0.1,stddev=0.001))
		...
		# after training
		# use the trained parameters to evaluate or test
		if not isTrain:
			get_data = tf.train.import_meta_graph('./Model/model-2.meta')
			print('sucessfully find meta file')
			get_data.restore(sess, tf.train.latest_checkpoint('Model/'))
			_, l = sess.run([optimizer, Loss],
						feed_dict={bu_OneHot: which_user, bp_OneHot: which_item, size_rating: [5 - 1],
								   e_pi: [3.1], R: [4]})
		...
		for iter_ in range(iter_cnt):
			...
			for i in sequence_key:
				...
				for j in range(len(to_training)):
					...
					_, l = sess.run([optimizer, Loss], feed_dict={bu_OneHot: which_user, bp_OneHot: which_item,size_rating:[i-1],e_pi: [user_product_pair[2]],R: [user_product_pair[3]]})
			...
			if iter_%2==0:
				...
				print('\n****************{}th training iterations loss is {}****************'.format(iter_, total_loss / (float)(count)))
				sess.run(lr_decay)
		log_file.close()
		sess.close()
	...
	def train(self):
		...
		if not os.path.exists('preprocess.pickle'):
			...
			self.user_ids,self.item_ids,self.g,self.Bu,self.Bp,sequence_info = pickle.load(f)
		self.train_model(sequence_info,True)
...
if __name__=="__main__":
	if not os.path.exists('HIALF.model'):
		HIALF().train()
	else:
		HIALF().test()
